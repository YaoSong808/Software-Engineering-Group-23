...
/**
 * This until class is to calculate user's balance, total income, expense, interest income, fixed deposit, etc.
 */
...
============  Fixed Deposit ============
当存一笔钱为定期存款时，会向transaction.csv文件插入6条记录，sample:
 event	                type	    eventvalue	    eventID	    userID	   createtime	    modifiedtime
...
FixedDeposit_percentage	interest	15%             900003	     321	   2024/2/6 16:40	2024/2/6 16:40

("open" 代表这笔钱当前是定期存款，没有到期，后面计算当前存款值时，只查询“open“对应的钱款（eventvalue)
(当存款到期还未到期，用户提前withdraw时，会向transaction.csv里，将type “open" 更新为"cancelled")

当存款到期，会向transaction.csv里，将type “Open" 更新为"Close"，同时会向transaction.csv里增加一条记录，sample:
 event	                type	    eventvalue	    eventID	    userID	   createtime	    modifiedtime
...
public class CalculateBalance {
    ...
    /**
     * Calculate specific user's total balance
     * @param csvFile specific csv file
    ...
    /**
     * Calculate specific user's total income
     * @param csvFile specific csv file
     * @param userId specific user's ID
     */
    // 计算收入
    ...
    /**
     * Calculate specific user's total expense
     * @param csvFile specific csv file
     * @param userId specific user's ID
     */
    //计算支出
    ...
    /**
     * Calculate specific user's total interest income
     * @param csvFile specific csv file
     * @param userId specific user's ID
     */
    //计算利息收益
    ...
    /**
     * Calculate specific user's total fixed deposit
     * @param csvFile specific csv file
     * @param userId specific user's ID
     */
    ...
    /**
     * Calculate specific user's total balance in latest one month
     * @param csvFile specific csv file
     * @param userId specific user's ID
     */
    // 计算一个月内的Balance变化
